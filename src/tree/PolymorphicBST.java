...
public class PolymorphicBST<K extends Comparable<K>, V>  {
	...
	public Set<K> keySet() {
		...
		/*
		 * PlaceKeysValuesInArrayLists<K,V> task=new
		 * PlaceKeysValuesInArrayLists<K,V>(); root.inorderTraversal(task);
		 * ArrayList<K> list=new ArrayList<K>(); int count=0;
		 * set=keySetAux(list,set,count);
		 */
	...
	private Set<K> keySetAux(ArrayList<K> list,Set<K> set,int count){
		...
		return keySetAux(list,set,count++);
	 ...
	 */
	 public K getMin() throws NoSuchElementException {
	...
	public PolymorphicBST<K, V> subMap(K fromKey, K toKey) {
		PolymorphicBST<K,V> subTree = new PolymorphicBST<K,V>();
		
				
			subTree.root=root.subTree(fromKey, toKey);
		return subTree;
	}
